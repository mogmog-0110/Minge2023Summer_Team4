...
void ObjectManager::createDebris()
{
	...
	while (myDebrises.size() < 30)
	{
		...
		GameObject* newDebris = ObjectAppearanceManager::createNewObject(eDebris, hp, myPlayer->getMaxHp() / 2, U"", Circle(hitbox), ObjectAppearanceManager::generateRandomPos(), { 0,0 }, { 0,0 });
		if (newDebris) {
