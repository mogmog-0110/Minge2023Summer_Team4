...
void ObjectManager::createDebris()
{
	...
	while (myDebrises.size() < 20)
	{
		...
		GameObject* newDebris = ObjectAppearanceManager::createNewObject(eDebris, myPlayer->getDamage() * 50, myPlayer->getHp() * 0.5, U"", Circle(hitbox), ObjectAppearanceManager::generateRandomPos(), {0,0}, {0,0});
		if (newDebris) {
...
void ObjectManager::createSpecialBullet(Vec2 pos, Vec2 vel, Vec2  acc)
{
	...
	case BulletType::SpecialD:
	{
		...
		GameObject* tempBullet = ObjectAppearanceManager::createNewObject(ePlayerBullet, 1, bp.damage + myPlayer->getDamage(), U"MineBullet", Circle{ 20 }, pos, vel.setLength(bp.speed), acc);
		if (tempBullet) {
			Bullet* newBullet = static_cast<Bullet*>(tempBullet);
